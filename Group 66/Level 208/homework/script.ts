...
function getMessage(value: string | number) {
...
let mixedArray: (string | number)[] = ['Nini', 8, 'Lalo', 12, 'Elene', 29];
...
// 6) შექმენით ტიპი type Status = "success" | "error" | "loading". შემდეგ შექმენით ფუნქცია showStatus(status: Status), რომელიც switch ოპერატორის გამოყენებით თითოეული შესაძლო მნიშვნელობისთვის დაბეჭდავს შესაბამის შეტყობინებას. გამოიძახეთ ფუნქცია სამივე შესაძლო მნიშვნელობით.
